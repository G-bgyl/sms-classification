...
def read_data(file_name,overide = False):
    ...
    if overide:
        ...
        with open(file_name) as sms_messages:
            reader = csv.reader(sms_messages)
            next(reader)
            for each in reader:
...
def cleanSentences(sentence):
    s = re.sub("[\s+\.\!\/_,$%^*(+\"\']+|[+——！，。？、~@#￥%……&*（）：；《）【】《》“”()»〔〕-]+", "", sentence) # .decode("utf8")
    return jieba.lcut(s)
            ...
            try:
                ...
                while len(word) > 0:
                    ...
                    if i == 0:
                        # Make sure the length of th list is less than maxSeqLength
                        if len(sent_idx_list) < maxSeqLength:
                            sent_idx_list.append(generate_new(word))
                            sent_str_list.append(word)
                            break
                        else: break
        else: break
...
# -----------------------------
def contextualize(raw_data, overide = False):
    '''
    ...
    if overide:
        data = pickle.load(open("DATA/final_data.p", "rb"))
        ...
        for each in raw_data:
            ...
            sent_str_list, sent_idx_list = sentence2sequence(string_content)
            ...
            i+=1
        pickle.dump(final_data, open("DATA/final_data.p", "wb+"))
        ...
        print("======== total spend for contextualize:",spend_time,'minutes ========')
...
def split_data(final_data,train_size = 0, overide = False):
    ...
    if overide:
        ...
        dev_data = pickle.load(open("DATA/dev_data.p", "rb"))
        ...
        train_data , dev_data = train_test_split(train_dev, test_size=0.25,random_state=42)
        ...
        pickle.dump(dev_data, open("DATA/dev_data.p", "wb+"))
...
def train(train_data,test_data,drop_out=0.75,overide = False):
    ...
    if overide:
        ...
        for i in range(iterations):
            ...
            accuracy_num, correctPred_list = sess.run([accuracy,correctPred],
                                                              feed_dict=feed_dict)
            train_accu = round((accuracy_num * 100), 5)
            if (i+1) % 50 ==0:
        ...
        feed_dict = {batch_vec_data: nextBatchVec, labels: nextBatchLabels}
        accuracy_num, correctPred_list, prediction_list = sess.run([accuracy, correctPred,prediction],
                                              feed_dict=feed_dict)
    ...
    '''
    # zip(*batch_data) returns labels, sent_str_list, sent_idx_list
    nextBatchLabels, nextBatchStr, nextBatchVec = zip(*test_data)
    feed_dict =  {batch_vec_data: nextBatchVec, labels: nextBatchLabels}
    accuracy_num, correctPred_list, prediction_list = sess.run([accuracy, correctPred,prediction],
                                          feed_dict=feed_dict)
    ...
    print("Accuracy for test data:", test_accu)
...
if __name__=='__main__':
    ...
    data_file = 'sms_messages.csv'
    zh_word_vec_file = 'zh_w2v.tsv'

    train_file = 'train_sms_messages.csv'
    dev_file = 'dev_sms_messages.csv'
    test_file = 'test_sms_messages.csv'
    ...
    train_data, dev_data, test_data = split_data(None, True)

    train(train_data,test_data,True)
